...
watchEffect(() => {
  if (
    ...
    console.log("debug", questions.value);
...
<template>
  <div class="wrapper-course-content">
    ...
    <div class="group-buttons">
      ...
      <button
        class="bttn bttn-drk quiz-btn go-quiz"
        v-if="
          chapters.length === parseInt(chapIndex) + 1 && questions.length > 0
        "
      >
      ...
      <button
        class="bttn bttn-wng quiz-btn next-chapter"
        v-else-if="chapters.length !== parseInt(chapIndex) + 1"
      >
        <RouterLink :to="`/course/${courseId}/${parseInt(chapIndex) + 1}`">
          Chapitre suivant
          <va-icon name="chevron_right" />
